...
- **Auto-buff does not consume the owner's MP.** The server applies the `BuffData` directly to the owner's `BuffComponent`, bypassing `AbilityComponent`. This is intentional MapleStory parity; manually-cast abilities still cost MP.
