...
your **recommended answer** so the user can react to a proposal instead of
...
skill exists. Then grill only the cross-cutting decisions the recipe won't
make for them (balance, integration with other systems, save-format
implications, what new infrastructure the content needs).
...
backend. Some answers are wrong by construction — flag them the same way
you'd flag a contradiction with the glossary: directly, with the rule named.
...
If the plan proposes new code for content that could be data, ask why.
Custom logic (e.g. `AL_*` ability scripts) is the exception, not the rule.
...
### Channels vs maps
A **channel** is a port-switched server instance (`ChannelManager`). A **map**
is a scene loaded within a channel (`MapManager`). Players say "channel,"
"world," "map," and "server" interchangeably — pin it down.
...
Single-context repo (one game). Don't look for `CONTEXT-MAP.md`.

Create `CONTEXT.md` and `docs/adr/` lazily — only when there's something
concrete to write. A grilling session that produces neither file is a valid
outcome.
...
- "skill" → **Ability.** (See Step 0 — there is no separate skill system.)
