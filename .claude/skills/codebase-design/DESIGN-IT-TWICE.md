...
- The constraints any new interface would need to satisfy (server authority,
  RPC shape, bot routing, persistence layer — pull these from the root
  [CLAUDE.md](../../../CLAUDE.md) and any relevant subsystem guide).
...
behind the seam, which RPC shape and persistence layer is in play). The
brief is independent of the user-facing problem-space explanation in Step 1.
...
this codebase (a `Node` child of the character root) and must not be reused
as a synonym for **Module**.

Each sub-agent outputs:

1. Interface (types, methods, params — plus invariants, ordering, error
   modes, RPC shape if cross-peer, signal contract if any)
2. Usage example showing how callers use it (server side, client side, and
   bot side if applicable — bots have no client, so route via autoload)
...
**locality** (where change concentrates), and **seam placement** (which seam
moves, and whether the new placement respects the server-authority boundary).
