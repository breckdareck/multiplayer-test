...
description: >-
  Shared vocabulary for designing deep modules in this Godot multiplayer RPG. Use
  when the user wants to design or improve a module's interface, find deepening
  opportunities, decide where a seam goes, make code more testable or
  AI-navigable, or when another skill (improve-codebase-architecture) needs the
...
Use these terms exactly — don't substitute "service," "API," or "boundary."
Consistent language is the whole point.

Note: in this Godot codebase, **Component** is also a *domain* term (see
[CONTEXT.md](../../../CONTEXT.md)) — a `Node` child of the character root. Don't
use **Component** as a synonym for **Module**. A character `Component` is a *kind
of* `Module`, but not every `Module` is a `Component`.

**Module** — anything with an interface and an implementation. Deliberately
scale-agnostic: a function, class, package, autoload, scene, or tier-spanning
slice. _Avoid_: unit, service. (Reserve **Component** for the domain term.)
...
configuration, and performance characteristics. In Godot this includes the RPC
shape, signal contract, group membership, expected node-tree position, and
autoload ordering. _Avoid_: API, signature (too narrow — they refer only to the
type-level surface).
...
- **In server-authoritative code, the seam is the wire.** The client sends intent;
  the server publishes authoritative state. A "deepening" that collapses that
  seam is not a deepening — it's a protocol change. Treat the server-authority
  boundary as a fixed seam, not a free design variable. Client and server are
  *always* two adapters (more, once bots are counted).
...
1. **Accept dependencies, don't create them.**
   ...
   ```gdscript
   # Testable
   func process_order(order, payment_gateway) -> Result: ...

   # Hard to test
   func process_order(order) -> Result:
       var gateway := StripeGateway.new()
   ```
...
2. **Return results, don't produce side effects.**
   ...
   ```gdscript
   # Testable
   func calculate_discount(cart) -> Discount: ...

   # Hard to test
   func apply_discount(cart) -> void:
       cart.total -= discount
   ```

3. **Small surface area.** Fewer methods = fewer tests needed. Fewer params =
   simpler test setup. The zero-dependency harness in `test/` exercises modules
   through their interfaces — that's the shape to aim for.
...
- **"Interface" as a GDScript class's public methods, or the `class_name` /
  exported-property surface**: too narrow — interface here includes every fact a
  caller must know (RPC shape, signal contract, group membership, expected
  node-tree position, autoload ordering).
...
- **Deepening a cluster given its dependencies** — see [DEEPENING.md](DEEPENING.md):
  dependency categories, seam discipline, replace-don't-layer testing, and the
  Godot-specific gotchas (autoloads, RPCs-as-interface, `.tres`-as-interface,
  signals, the `networked_entities` group).
- **Exploring alternative interfaces** — see
  [DESIGN-IT-TWICE.md](DESIGN-IT-TWICE.md): spin up parallel sub-agents to design
  the interface several radically different ways, then compare on depth, locality,
  and seam placement.
