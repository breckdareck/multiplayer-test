...
the more editorial visuals (mass diagrams, cross-sections). Mix the two —
don't lean on Mermaid for everything, it'll start to look generic.

## Where to write it

- **Windows / PowerShell** (this project): `$env:TEMP\architecture-review-<timestamp>.html`, open with `Start-Process <path>`.
- **macOS**: `$TMPDIR/architecture-review-<timestamp>.html`, open with `open <path>`.
- **Linux**: `/tmp/architecture-review-<timestamp>.html`, open with `xdg-open <path>`.

Always tell the user the absolute path after writing.
...
- **Title** — short, names the deepening (e.g. "Collapse the Pet auto-loot
  pipeline").
- **Badge row** — recommendation strength (`Strong` = emerald, `Worth
  exploring` = amber, `Speculative` = slate), plus a tag for the dependency
...
- **Wins** — bullets, ≤6 words each. e.g. "Tests hit one interface", "Pet
  position clamp stops leaking", "Delete 4 shallow wrappers".
- **ADR callout** (if applicable) — one line in an amber-tinted box
  referencing the ADR by number, e.g. *"contradicts ADR-0001 — but worth
  reopening because…"*.
...
Use a Mermaid `flowchart` or `graph` when the point is "X calls Y calls Z,
and look at the mess." Wrap it in a Tailwind-styled card so it doesn't feel
parachuted in. Style with `classDef` to colour leakage edges red and the
deep module dark. Sequence diagrams work well for "before: 6 round-trips;
after: 1."
...
<div class="rounded-lg border border-slate-200 bg-white p-4">
  <pre class="mermaid">
    flowchart LR
      A[PetManager] --> B[Pet.gd]
      B --> C[AbilityComponent]
      C -.leak.-> D[PlayerSave]
...
### Sequence diagram for RPC round-trips

When the candidate is about cross-peer chatter (client → server → broadcast
→ client), a Mermaid `sequenceDiagram` shows the cost cleanly. Before: many
arrows. After: one intent + one broadcast.
...
<pre class="mermaid">
  sequenceDiagram
    ...
    C->>S: cast_ability_request(id)
    S->>S: validate, mutate, schedule
    S-->>C: ability_cast_authoritative(...)
    S-->>P: ability_cast_authoritative(...)
</pre>
```
...
**Never substitute:** service, unit (for module) · API, signature (for
interface) · boundary (for seam) · layer, wrapper (for module, when you mean
module). **Component** is reserved for the domain meaning — don't reuse it
for **module**.

**Phrasings that fit the style:**

- "Pet lifecycle module is shallow — interface nearly matches the
  implementation."
- "Combat leaks across the Stats seam."
- "Deepen: one interface, one place to test."
- "Two adapters justify the seam: ENet in prod, in-memory in tests."
...
maintain"* or *"cleaner code"* — those terms aren't in the glossary and don't
earn their place.
